...
def reinforce_prune_train(model, dataset, reward_fn, epochs=10, batch_size=1, lr=1e-3, 
                         beta=0.99, min_guards=3, coverage_threshold=0.99, use_cache=True):
    """
    ...
    Args:
        use_cache: Use precomputed visibility regions for fast coverage computation
    """
    print(f"\n--- Pruning Training on {len(dataset)} samples for {epochs} epochs (batch size {batch_size}) ---")
    
    # Initialize visibility cache if enabled
    vis_cache = None
    if use_cache:
        vis_cache = get_global_cache()
        print("Precomputing visibility regions for training dataset...")
        vis_cache.precompute_dataset(dataset, show_progress=True)
        stats = vis_cache.get_stats()
        print(f"Cached {stats['instances_cached']} instances in {stats['total_precompute_time']:.1f}s")
        ...
        for batch_data, mask, lengths_tensor, batch_names in pbar:
            ...
            for b_idx in range(batch_data.size(0)):
                ...
                for rem_idx in removal_idxs:
                    ...
                    # Check coverage - use cache if available
                    if vis_cache is not None:
                        coverage = vis_cache.get_coverage_fast(points, np.array(candidate_guards), name)
                    else:
...
def evaluate_pruning_policy(model, dataset, reward_fn, batch_size=1, min_guards=3, 
                           coverage_threshold=0.99, sol_dir=None, use_cache=True):
    """
    ...
    if use_cache:
        vis_cache = get_global_cache()
        # Precompute validation set if not already cached
        if len(vis_cache.guard_visibility_cache) == 0:
            print("Precomputing visibility regions for validation dataset...")
            vis_cache.precompute_dataset(dataset, show_progress=True)
        else:
            print(f"Using cached visibility regions ({vis_cache.get_stats()['instances_cached']} instances)")
        # Clear coverage cache for fresh evaluation
        vis_cache.clear_coverage_cache()
    ...
    with torch.no_grad():
        ...
        for idx in pbar:
            ...
            if vis_cache is not None:
                final_coverage = vis_cache.get_coverage_fast(points, final_solution, name)
            else:
    ...
    # Print cache statistics
    if vis_cache is not None:
        cache_stats = vis_cache.get_stats()
        print(f"\nCache Statistics:")
...
def main():
    ...
    parser.add_argument('--no-cache', action='store_true',
                       help='Disable visibility region caching (slower but uses less memory)')
    ...
    print(f"  Visibility caching: {'enabled' if use_cache else 'disabled'}")
