...
Both targets come from the same `paper.tex`. `paper-arxiv.tex` is generated by
`paper/scripts/build_arxiv_tex.py` -- a self-contained flattening (not an `\input`
wrapper), so the arXiv submission bundle has exactly one file with `\documentclass`
and needs no `00README.XXX` toplevel override. Regenerate it after editing
`paper.tex`. Neither target needs any data outside the repository.
