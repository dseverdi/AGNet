...
To avoid the dev-overfit failure mode, the standard `dev/` split is partitioned **deterministically** into:

- **`dev_tune`** — 857 polygons (≈ 70 %) used for threshold selection and any hyperparameter decisions.
- **`dev_test`** — 367 polygons (≈ 30 %) **held-out**, never seen during tuning. *Headline numbers in this section are reported on `dev_test` unless stated otherwise.*

The pretrained pointer (`lstm_bt`) was trained on `train/` only; the SetPredictor was trained on `(pointer-seed, LS-final)` pairs derived from `train/` polygons. Thresholds for the headline operating points (`t = 0.30`, `t = 0.25`, `t = 0.20`) were chosen on `dev_tune` and confirmed on `dev_test`.

The partition was generated by `tools/split_dev_pickle.py` (alphabetical-by-name 70/30 in this run; the script also supports a seeded-random shuffle for partition rebalancing). The alphabetical split clusters polygon types (`fat-*` polygons concentrate in `dev_tune`, `randsimple-*` in `dev_test`), creating a mild distribution gap: `dev_tune` seed `|S|/OPT = 1.27`, `dev_test` seed `|S|/OPT = 1.09`. Seed coverage is comparable across the two (~0.97).

**Evaluation status:**

| Evaluation | Status | Notes |
|---|---|---|
| Held-out in-distribution (`dev_test`, 367 polygons) | ✅ complete | Headline numbers below |
| Out-of-distribution (`test/`, 2107 polygons, n ≤ 1000) | ⏳ pending | Trajectory build queued |
| Extreme OOD (`large/`, 285 polygons, n ≤ 2250) | ⏳ optional | If time permits |
| Classical comparison (greedy AGP on dev) | ⏳ pending | One short run |

### Headline Result — Held-Out 367 Dev Polygons
...
| Pointer EOS-disabled (ref. Approach 15) | 0.992    | (gap)     | 0.323   | 2.18      | (many)           | (few)    | (few)   |
| Pointer + full LS (gold, with `disc_vis`) | 0.976  | —         | 0.148   | 0.91      | —                | —        | —       |

**Headline claim** *(on 367 held-out dev polygons)*:

> "At threshold `t = 0.30`, the SetPredictor achieves coverage ≥ 0.95 on **every one** of the 367 held-out dev polygons (Wilson 95 % CI ≤ 1.0 %). The pointer baseline alone fails this criterion on **71 / 367 (19.3 %)** of held-out polygons. Mean coverage rises from 0.969 → **0.998**, worst-case coverage rises from 0.83 → **0.95**. Cost: |S|/OPT 1.09 → 2.20 (single neural forward per polygon, no geometric oracle at inference)."

Three findings carry the paper:

1. **The editor at `t = 0.30` achieves the "no polygon below 95 %" guarantee on every held-out polygon** with worst-case cov 0.952 and |S|/OPT 2.20. The pointer baseline fails on 19.3 % of held-out polygons. *Same 367 polygons, same metric.*
2. **The editor at `t = 0.30` strictly Pareto-dominates "let the pointer sample more"** (the pointer-EOS-disabled baseline, Approach 15 in `plan.md`). Higher mean coverage (0.998 vs 0.992) AND fewer guards (|S|/OPT 2.20 vs 2.18). This answers the natural reviewer challenge — *"why not just have the policy pick more vertices?"* — with measurement.
3. **The threshold `t` is a single inference-time scalar that walks a smooth Pareto curve.** Same trained model exposes three operating points without retraining: coverage-guaranteed (`t = 0.20`), balanced (`t = 0.30`), efficient (additional thresholds available). The user chooses the deployment trade-off.

### Why "Mean Coverage" Hides the Story — The Per-Polygon Distribution

The headline above is striking because the **pointer's mean coverage** (0.969 on dev_test) is misleadingly high — it averages many partial failures with a small population of fully-covered polygons. The per-polygon distribution makes this explicit:

| Coverage band       | Pointer alone (dev_test) | Pointer + SetPredictor (`t = 0.30`, dev_test) |
|---                  |---:                      |---:                                            |
| cov = 1.0           | 3 / 367  (0.8 %)         | 152 / 367 (41 %)                              |
| 0.999 ≤ cov < 1.0   | 12 / 367 (3.3 %)         | 69 / 367  (19 %)                              |
| 0.99  ≤ cov < 0.999 | 84 / 367 (23 %)          | 134 / 367 (37 %)                              |
| 0.95  ≤ cov < 0.99  | 197 / 367 (54 %)         | 12 / 367  (3.3 %)                             |
| cov < 0.95          | **71 / 367 (19.3 %)**    | **0 / 367**                                   |
| Worst polygon       | cov = 0.83               | cov = 0.95                                    |

The editor lifts essentially the entire dev_test population into the cov ≥ 0.99 band. **Every one of the 71 pointer-failures (cov < 0.95) is rescued.** This is a *distributional shift*, not a mean-improvement.

### Earlier Full-Dev Evaluation (Context — Not the Held-Out Claim)

Before the held-out methodology was put in place, the same SetPredictor checkpoint was evaluated on the **full** 1224-polygon dev set (which mixes the partition that's now `dev_tune` with what's now `dev_test`). Those numbers are kept here for reference and reproducibility. They are **not** the headline claim because the threshold was selected on the same set.

| Method (full dev, 1224 polygons)              | cov   | \|S\|/n | \|S\|/OPT | # < 0.95         |
|---                                            |---:   |---:     |---:       |---:              |
| Pointer alone                                 | 0.970 | 0.194   | 1.21      | 176/1224 (14.4 %) |
| + SetPredictor `t = 0.30`                     | 0.995 | 0.331   | 2.09      | 1/1224 (0.08 %)   |
| + SetPredictor `t = 0.20`                     | 0.999 | 0.461   | 2.92      | 0/1224            |
| Pointer + full LS (gold)                      | 0.976 | 0.148   | 0.91      | —                 |

The directional finding is the same in both evaluations; the held-out version is the methodologically clean one to cite.

### Controllable Pareto Curve

A single trained model exposes a Pareto frontier via the threshold `t`. Three named operating points are highlighted; **all numbers on dev_test (held-out, 367 polygons)**:
...
All three are achievable from one trained model, switched at inference by a single scalar. No retraining.

### How the Pointer and Editor Compose

The pointer is **not subsumed** by the SetPredictor. Each method occupies a distinct niche:

| If you want                            | Use                              |
|---                                     |---                               |
| Lowest inference cost, OK coverage     | **Pointer alone** (1 forward, no second model) |
| High coverage on every polygon         | **Pointer + SetPredictor (t=0.30 or 0.20)**    |
| Minimum-guard exact (with cost)        | **Pointer + LS** (uses disc_vis at every step) |

The pointer also serves as the SetPredictor's input source: its **encoder embeddings** (128-d per vertex, computed once per polygon from coordinates) and its **greedy seed set** are both used as features. Removing the pointer would reduce the SetPredictor to learning polygon visibility from raw (x, y) — exactly the formulation that failed in three prior iterative-editor variants (see "Why Single-Shot" below).
...
- **Output**: per-vertex sigmoid `P(v ∈ S_final)`. Final set = `{v : P ≥ t}`.
...
- Best result at epoch 28 (training on standard τ=0.99 trajectories, 60 epochs total).

### Why Single-Shot Was the First Approach That Worked

Three prior **iterative-editor** attempts (v1: 80 k params; v2: 600 k + topology + aux loss; v3: strict-τ + strong STOP head) all failed with the same family of pathologies:

1. **Compounding errors.** One wrong REMOVE early in a rollout destroys downstream coverage.
2. **STOP/action coupling.** The STOP head decides when to halt; the action head decides what to edit. Their errors interact catastrophically — model either over-prunes (cov collapses) or under-prunes (no improvement).
3. **Training/inference distribution drift.** Editor visits clean LS states during training but messy editor-rollout states at inference. DAgger partially closes this but cannot bridge the gap from coordinates alone.

The single-shot formulation eliminates all three by construction:

1. **No iteration** → no error compounding. Wrong on vertex 7 doesn't affect prediction on vertex 12.
2. **No STOP head** → the confidence threshold replaces STOP. Per-polygon, per-threshold; no STOP/action interaction.
3. **Same input distribution** at train and inference: both are `(polygon, pointer_seed)`. The model sees exactly the input it will see at deployment. No drift possible.

Iterative refinement of the SetPredictor's own output (feeding `S_pred` back as `in_S` for K=2, 3, 5 more passes) was also tested. **The model converges in one step**: K=2/3/5 produce nearly identical predictions to K=1, confirming that single-shot is the right formulation (not a workaround).

### Comparison Against the Closest Neural Alternative

The most natural alternative to the SetPredictor is **the pointer with EOS disabled** — let the policy keep picking guards until it reaches some coverage threshold (a setting documented earlier in `plan.md` as "Approach 15"). This is the "sample more from policy" approach.

| Method                              | mean cov | \|S\|/OPT |
|---                                  |---:      |---:       |
| Pointer EOS-disabled (Approach 15)  | 0.992    | 2.18      |
| **Editor at t=0.30**                | **0.9953** | **2.09** |

The editor produces **higher coverage with fewer guards** than the natural alternative — a strict Pareto improvement of ~4 % on `|S|/OPT` at the same coverage band. This is the editor's main quantitative contribution.

### Reproducing the Best Result (Held-Out Methodology)

```bash
# 1. Build LS imitation targets (standard τ=0.99 trajectories, ~1h wall, run once)
...
# 2. Train the SetPredictor (~5 min for 60 epochs)
...
# 4. Pick thresholds on the tune partition (~15 min)
...
# 5. Held-out eval at the chosen thresholds (~3 min) — this is the headline number
python eval_set_predictor.py \
    --checkpoint checkpoints/set_predictor/standard/set_predictor_best.pt \
...
# 6. (Pending) OOD eval on `test/` polygons (vertex count up to 1000) — ~45 min trajectory + ~10 min eval
...
- `eval_set_predictor.py` — standalone evaluation: per-polygon coverage distribution metrics, optional iterative-inference sweep
- `tools/split_dev_pickle.py` — deterministic 70/30 carve of dev trajectory pickle into tune/held-out partitions
- `tools/build_ls_trajectories.py` — supports `--split {train,dev,test,large}` to build LS-imitation targets on any split
...
### Architecture choice: LSTM pointer (not Transformer)

The pretrained pointer is an LSTM PointerNet (~364 k params), not a Transformer. The Pan et al. (ICML 2025) PO paper which motivated this work uses Transformer architectures (POMO, AM, Pointerformer) exclusively. The natural reviewer challenge: *"would Transformer + PO fine-tuning have worked end-to-end where LSTM + PO failed?"*

**We trial-implemented a Transformer pointer:**
- **5 Mar 2026** (commit `62002b2`): added a `TransformerPointerNet` (3-layer MHA encoder, 8 heads, FFN 128→512→128, ≈ 1.8 M params — matching AM dimensions) alongside the LSTM via a `model_type` config flag. 337 LoC in `models.py`.
- **12 Mar 2026** (commit `99dc2bd`): the Transformer code path was removed and the model factory was simplified back to LSTM only. 337 LoC reverted from `models.py`.

The reversion happened ahead of the editor-direction pivot. Initial Transformer training runs were not stable under the same PO/BT loss settings that worked for the LSTM. Rather than tune the Transformer end-to-end (a multi-day effort), the work was redirected toward the editor approach which **adds** capability on top of the existing LSTM pointer without architectural risk to the established baseline. The Transformer code is recoverable from git history at any time; we leave a clean Transformer + PO comparison as future work.

This does mean some of our negative findings (the slide-along-the-trade-off-curve under FT, the iterative editor pathologies) may not all generalize to a Transformer pointer architecture. The single-shot SetPredictor finding (this README's headline result) is **architecture-orthogonal**: it consumes encoder embeddings as features and would work with either a Transformer or an LSTM pointer.

### Honest Caveats

- **Below classical LS on guard efficiency.** At any matched coverage band, classical LS (with `disc_vis` at every step) produces materially smaller sets. At cov ≈ 0.97, LS achieves `|S|/OPT = 0.91`; the SetPredictor at the same coverage achieves `|S|/OPT ≈ 1.18`. The neural method buys *inference speed* (no per-step coverage queries) and *deployment simplicity* (no geometric oracle), not raw guard efficiency.
- **No strict Pareto improvement over the pointer alone exists** at any tested threshold on dev_test. The editor produces a calibrated Pareto curve; the pointer's natural operating point (cov 0.969, |S|/OPT 1.09) is on the curve, not below it. The editor's value is **coverage quality** (lifting the cov-failure tail to zero), not **guard reduction at preserved cov**.
- **Held-out partition size: 367 polygons.** Wilson 95 % CI for "0 polygons below cov 0.95 out of 367" is `[0 %, 1.0 %]`. The OOD `test/` evaluation, when complete, will provide a second held-out evidence base of 2107 polygons.
- **Partition is alphabetical 70/30, not random-shuffled.** This concentrates polygon types (`fat-*` in tune, `randsimple-*` in test). Seed `|S|/OPT` differs 1.27 vs 1.09 between partitions. The held-out claim still survives because the threshold was tuned only on `dev_tune` and reported on `dev_test`. A seeded-random shuffle option exists in `tools/split_dev_pickle.py` for partition rebalancing.
- **Earlier "Δ|S|/n at preserved cov" framing was a small-sample artifact.** Per-epoch eval during training used 100 polygons (first by size bucket) and reported a cov-preserving improvement that did not survive evaluation on the full 1224-polygon dev set. The held-out methodology described above prevents the same error.
- **Iterative inference of the SetPredictor doesn't help.** K = 2, 3, 5 passes converge to nearly identical predictions to K = 1. The model is a fixed point under self-iteration.
- **High-coverage operating points cost real guards.** `t = 0.20` achieves the "no polygon below 95 %" guarantee but at |S|/OPT = 2.90 — roughly 2.7× the pointer's natural guard count. This trade-off is inherent to AGP: coverage requires guards.
- **`test/` and `large/` are OOD, not in-distribution.** `test/` extends to 1000 vertices (5× train max); `large/` extends to 2250 vertices (11× train max). Headline numbers in this section come from a deterministic 70/30 carve of `dev` (`dev_tune` for tuning, `dev_test` for held-out reporting), all polygons ≤ 198 vertices. The `test/` and `large/` folders are reserved for separate generalization evaluations (pending).
- **Pointer-baseline generalization to large polygons is unmeasured.** The LSTM PointerNet was trained on polygons ≤ 198 vertices. Performance on `test/` (up to 1000) and `large/` (up to 2250) is theoretically expected to degrade due to (a) attention softmax flattening over many more candidates, (b) sequential hidden-state degradation over longer decode sequences, (c) EOS calibration drift, but is not yet measured. The pending OOD evaluation will report this directly.
