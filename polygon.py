...
class Point2(skgeom.Point2):
    def __hash__(self):
        return (float(self.x()), float(self.y())).__hash__()
...
class Polygon:
    def __init__(self: Type["Polygon"], filename: Optional[str] = None, poly: Optional[skgeom.Polygon] = None):
        ...
        self.poly = poly
        # vertices        
        self.Vertices = [Point2(v.x(), v.y()) for v in self.poly.vertices]  # Use custom Point2
    ...
    def __init_vis_queries(self: Type["Polygon"]) -> None:
        ...
        for i in range(n):
            ...
            q = v + eps * (p + r)
            # change orientation if the point is outside of the polygon
            ...
            # compute visibility region for q as a polygon
            self.view[v] = skgeom.Polygon([_.point() for _ in vx.vertices])

    def read_pol(self: Type["Polygon"], filename: str) -> skgeom.Polygon:
        """
        ...
        with open(filename, 'r') as f:
            ...
            verts = [Point2(scale(p[0], l, r), scale(p[1], b, t)) for p in _points]

        return skgeom.Polygon(verts)

    def check_visibility(self: Type["Polygon"], p: Point2, q: Point2) -> bool:
    ...
    def discretize(self: Type["Polygon"]) -> float:
        ...
        # add edges from visibility polygons to the avp arrangement        
        ...
        self.Points = [Point2(1.0 / len(avp) * sum([p.x() for p in avp.vertices]), 1.0 / len(avp) * sum([p.y() for p in avp.vertices])) for avp in self.avps]

        return len(self.Points)

    def show(self: Type["Polygon"],show_discretization: Optional[bool] = False) -> None:
        """
...
class AGSolver:
    ...
    def __init__(self: Type["AGSolver"], P: Type["Polygon"]):
        """
        Define variables and constraints from the terrain data points
        """
        # verbose
        self.verbose = False
        # polygon
    ...
    def solve(self):
        ...
        self.solutions = [s for s in self.solutions if len(s) <= round(self.m.ObjVal)]
