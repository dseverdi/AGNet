"""Continuous Art Gallery Problem — guards at any interior point, not just vertices.

Guards and witnesses are both the equidistant interior grid points.
ILP finds the minimum subset of grid points that collectively see all grid points.
"""
...
def _get_interior_points(room: np.ndarray, spacing: float = 1.0) -> np.ndarray:
    """Generate equidistant interior grid points with given spacing."""
    xs = room[:, 0]
    ys = room[:, 1]
    ...
    inside = np.array([_point_in_polygon(p, room) for p in grid])
    return grid[inside]
...
def build_coverage_matrix(
    ...
    witness_spacing: float = 0.5,
    eps: float = 1e-7,
) -> tuple[np.ndarray, np.ndarray]:
    """Build coverage matrix: guards from candidates, witnesses from a denser grid.

    Returns (coverage [n_candidates x n_witnesses], witnesses [n_witnesses x 2]).
    """
    witnesses = _get_interior_points(room, spacing=witness_spacing)
    n_cand = candidates.shape[0]
    n_wit = witnesses.shape[0]

    C = np.zeros((n_cand, n_wit), dtype=int)
    for i in range(n_cand):
        vp = compute_visibility_polygon(room, candidates[i], eps=eps)
        if vp.shape[0] < 3:
            continue

        vp_shapely = ShapelyPolygon(vp)
        for j in range(n_wit):
            C[i, j] = int(vp_shapely.contains(ShapelyPoint(witnesses[j])) or
                          vp_shapely.boundary.distance(ShapelyPoint(witnesses[j])) < 1e-6)
    return C, witnesses


def solve_agp_ilp(coverage: np.ndarray) -> np.ndarray:
    """Solve minimum guard set cover ILP. Returns indices into the candidate array."""
    n_guards, n_witnesses = coverage.shape
    prob = LpProblem("AGP_continuous", LpMinimize)
...
def create_dataset(
    ...
    """Generate rooms, solve continuous ILP, save as .pt chunk files.

    Saves chunks of `chunk_size` rooms to data/<ds_name>_continuous/chunk_0000.pt, etc.
    Each datum dict:
        {"vertices": FloatTensor(N, 2),
         "candidates": FloatTensor(M, 2),
         "guards": LongTensor(K,),
         "n_vertices": int, "n_candidates": int, "n_guards": int,
         "solve_time": float}
    """
    ...
    for ds_name, cfg in DATASET_CONFIGS.items():
        ds_dir = output_dir / f"{ds_name}_continuous_{chunk_size}"
        ...
        for i, room in pbar:
            ...
            candidates = _get_interior_points(room, spacing=spacing)
            C, _ = build_coverage_matrix(room, candidates, witness_spacing=spacing / 2)
            ...
            pbar.set_postfix(verts=room.shape[0], cands=candidates.shape[0],
                             guards=len(guards), t=f"{solve_time:.2f}s")
...
def concat_dataset(data_dir: str | Path = "data", chunk_size: int = 1000) -> dict[str, Path]:
    ...
    for ds_name in DATASET_CONFIGS:
        ds_dir = data_dir / f"{ds_name}_continuous_{chunk_size}"
        ...
        out_path = data_dir / f"{ds_name}_continuous_{chunk_size}.pt"
...
def plot_dataset(
    ...
    image_dir: str | Path = "data/images/optimal_continuous",
    ...
    for ds_name in DATASET_CONFIGS:
        folder_name = f"{ds_name}_continuous_{chunk_size}"
        ...
        for i, datum in enumerate(dataset[:n], start=1):
            ...
            candidates = datum["candidates"].numpy()
            guard_indices = datum["guards"].numpy()

            out = ds_dir / f"{i}.png"
            _render_one(room, candidates, guard_indices, out, bounds)
            paths.append(out)
            print(f"Plotted {out}")
...
def _render_one(
    ...
    guard_points = candidates[guard_indices]

    # Visibility polygon per guard, clipped to room.
    for gi, gpt in enumerate(guard_points):
        strong, pale = GUARD_COLORS[gi % len(GUARD_COLORS)]
        vp = compute_visibility_polygon(room, gpt)
        if vp.shape[0] >= 3:
            vp_shapely = ShapelyPolygon(vp)
    ...
    # Room vertices.
    ax.scatter(room[:, 0], room[:, 1], s=14, c=colors.blue, zorder=3)

    # Non-guard candidate points (small, faint).
    non_guard_mask = np.ones(candidates.shape[0], dtype=bool)
    non_guard_mask[guard_indices] = False
    if non_guard_mask.any():
        ax.scatter(candidates[non_guard_mask, 0], candidates[non_guard_mask, 1],
                   s=0.5, c=colors.darkgray, zorder=2)

    # Guard points (stars).
    for gi, gpt in enumerate(guard_points):
        strong, _ = GUARD_COLORS[gi % len(GUARD_COLORS)]
        ax.scatter([gpt[0]], [gpt[1]], s=50, c=strong, zorder=5, marker="*")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Continuous AGP dataset creation & plotting")
    ...
    p_create.add_argument("-s", "--spacing", type=float, default=1.0)
    ...
    p_plot.add_argument("-o", "--image-dir", type=str, default="data/images/optimal_continuous")
    ...
    if args.command == "create":
        create_dataset(output_dir=args.output_dir, n_rooms=args.n_rooms,
                       spacing=args.spacing, chunk_size=args.chunk_size)
