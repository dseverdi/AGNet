...
def tab_headline() -> None:
    ...
    lines = [
        ...
        _row("\\quad SetPredictor, no-encoder ($t=0.20$)",
             noenc_agg, n),
        "  \\bottomrule",
        "\\end{tabular}",
        "",
...
def tab_ood() -> None:
    ...
    noenc = load("dist_test_OOD_noenc.json")["polygons"]
    ...
    lines = [
        ...
        _multiseed_row("SetPredictor full ($t=0.20$)",
                       ms["summary"], "probe_t020", n, bold_cov=True),
        _row("SetPredictor, no-encoder ($t=0.20$)", noenc_agg, n),
        "  \\bottomrule",
        "\\end{tabular}",
        "",
    ]
...
def tab_large() -> None:
    ...
    lines = [
        ...
        f"  SetPredictor, no-encoder ($t=0.20$) & {ne['cov']:.4f} & "
        f"{ne['cov_min']:.3f} & {feas(ne['n_below_095'])} & {ne['S_n']:.4f} \\\\",
        "  \\bottomrule",
        "\\end{tabular}",
        "",
    ]
