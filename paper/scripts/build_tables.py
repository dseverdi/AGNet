...
def _row(name: str, agg: dict, n_total: int, *,
    ...
    if agg["mean_S_opt"] is None:
        ...
        if sopt_dagger:
            # \tablefootnote, not \footnote: a plain \footnote inside a float is
            # silently dropped by LaTeX. Kept here rather than in the caption so the
            # marker sits on the cell it explains.
            sopt_s += (
                "\\tablefootnote{The oracle editor halts at the disc-vis gate "
                "($\\tau = 0.99$) and slightly undercovers, so it spends fewer "
                "guards than the \\emph{full-coverage} optimum, an artifact of "
                "the training reward, not a loose $\\OPT$.}")
...
def tab_dist_shift() -> None:
    ...
    n = len(full)
    # Only the headline threshold. The t=0.25 and t=0.30 rows were dropped once
    # fig:operating_curve took over the threshold sweep: they showed the gate
    # ladder shifting with t, which the figure now traces over the whole range.
    # What this table uniquely carries is the SHAPE of the distribution at one
    # operating point -- the exact-coverage and 0.999 columns, which no figure
    # or other table reports.
...
def tab_feature_baseline() -> None:
    """Linear readouts of four geo-free feature sets (Results, probe capacity).

    The companion control to the capacity ladder: the ladder holds features fixed
    and varies the probe, this holds the probe fixed and varies the features. Every
    arm is computable from vertex coordinates alone -- no visibility oracle -- so all
    four are geo-free. Source is tools/geometric_baseline_probe.py, which copies the
    cross-validated protocol of build_encoder_linear_probe verbatim; its `embedding`
    arm re-derives that rung and agrees to three decimals (0.8430 vs 0.8430; the
    per-fold values differ in the fourth, consistent with embedding extraction on a
    different device), which is what makes the rows comparable.
    """
    d = load("geometric_baseline_probe.json")["arms"]
    rows = [("Coordinates $(x_i, y_i)$", "coords"),
            ("Analytic geometry", "geometry"),
            ("Frozen encoder $h_i$", "embedding"),
            ("Encoder $+$ geometry", "embedding+geometry")]
    lines = ["% auto-generated by build_tables.py from geometric_baseline_probe.json",
             "\\begin{tabular*}{0.96\\textwidth}{@{\\extracolsep{\\fill}}lrrr}", "  \\toprule",
             "  Linear readout over & dim & ROC-AUC & PR-AUC \\\\", "  \\midrule"]
    for label, key in rows:
        a = d[key]
        bold = "\\textbf{%s}" % f"{a['roc_auc_mean']:.3f}" \
            if key == "embedding+geometry" else f"{a['roc_auc_mean']:.3f}"
        lines.append(
            f"  {label} & {a['n_features']} & "
            f"{bold}\\,$\\pm$\\,{a['roc_auc_std']:.3f} & "
            f"{a['pr_auc_mean']:.3f}\\,$\\pm$\\,{a['pr_auc_std']:.3f} \\\\")
    lines += ["  \\bottomrule", "\\end{tabular*}"]
    write("tab_feature_baseline.tex", "\n".join(lines))
...
if __name__ == "__main__":
    ...
    tab_feature_baseline()   # feature-set control for the capacity ladder
